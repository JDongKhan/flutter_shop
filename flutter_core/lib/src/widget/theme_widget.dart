...
typedef ThemeWidgetBuilder<T extends ChangeNotifier> = Widget Function(
    BuildContext context, T controller);

class ThemeNotifierProviderWidget<T extends ChangeNotifier>
...
//    final provider =  context.inheritFromWidgetOfExactType(type) as JDInheritedProvider<T>;
    final provider =
        context.dependOnInheritedWidgetOfExactType<ThemeProvider<T>>();
...
class _ThemeNotifierProviderState<T extends ChangeNotifier>
    extends State<ThemeNotifierProviderWidget<T>> {
  void update() {
...
class ThemeProvider<T> extends InheritedWidget {
  const ThemeProvider({super.key, required this.data, required Widget child})
      : super(child: child);
  ...
  @override
  bool updateShouldNotify(ThemeProvider<T> old) {
