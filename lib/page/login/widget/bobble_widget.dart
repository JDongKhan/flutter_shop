...
class BobbleWidget extends StatefulWidget {
  ...
  @override
  State createState() => _BobbleWidgetState();
}
...
class _CustomMyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    list?.forEach((element) {
      Offset newCenterOffset =
          _calculateXY(element.speed ?? 0, element.theta ?? 0);

      double dx = newCenterOffset.dx + (element.position?.dx ?? 0);
      double dy = newCenterOffset.dx + (element.position?.dy ?? 0);

      if (dx < 0 || dx > size.width) {
        dx = (random?.nextDouble() ?? 0) * size.width;
      }

      if (dy < 0 || dy > size.height) {
        dy = (random?.nextDouble() ?? 0) * size.height;
      }
    ...
    list?.forEach((element) {
      _paint.color = element.color ?? Colors.white;
      canvas.drawCircle(
          element.position ?? Offset.zero, element.radius ?? 0, _paint);
    });
  }
